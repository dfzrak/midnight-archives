...
function showPay(k){var ex=document.querySelector('.po');if(ex)ex.remove();var p=CP[k];var oid='MA-'+Date.now().toString(36).toUpperCase();_currentOrder={k:k,oid:oid};var ov=document.createElement('div');ov.className='po';ov.innerHTML='<div class="pm"><div class="px">✕</div><div class="ph"><div class="pi">'+(k==='huge'?'🔮':k==='large'?'💰':'🪙')+'</div><h3>'+p.n+'</h3><p>'+p.d+'</p></div><div class="pd"><div class="pr"><span>获得</span><span>+'+p.c+' 🧱</span></div><div class="pr"><span>支付</span><span class="pp">¥'+p.p+'</span></div><div class="pr pro"><span>订单号</span><span class="poi">'+oid+'</span></div></div><div class="pq"><img src="../assets/alipay-qr.jpg" alt="收款码" class="pqi" onerror="this.style.display=\x27none\x27"><p class="pqh">📱 支付宝扫一扫</p><p class="pqs">⚠ <strong>扫码后请等待页面自动确认</strong></p></div><div class="pa"><button class="bv" data-k="'+k+'" data-o="'+oid+'">✅ 已完成支付</button><button class="bc2">取消</button></div></div>';ov.querySelector('.bc2').onclick=function(){ov.remove();_currentOrder=null};ov.querySelector('.px').onclick=function(){ov.remove();_currentOrder=null};ov.querySelector('.bv').onclick=function(){var btn=this;btn.disabled=true;btn.textContent='⏳ 正在同步…';submitOrder(k,oid,'confirmed');setTimeout(function(){ov.remove();toast('✅ 订单已确认。书记员正在验证 ¥'+p.p+' 付款，收到后即刻发放。',5000);},800);};ov.addEventListener('click',function(e){if(e.target===ov){ov.remove();_currentOrder=null}});document.body.appendChild(ov);setTimeout(function(){document.getElementById('pvi')&&document.getElementById('pvi').focus()},500);}
...
console.log('[Archive] 充值系统 v11 | 余额',bal,'🧱 | 手动验证模式 | 无自动发放');
})();