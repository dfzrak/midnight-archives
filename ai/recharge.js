...
function renderRecharge(cid){
 ...
 injCss();
 syncBalanceFromGist(function(ok,newBal){if(ok){var be=document.getElementById('bbal');if(be)be.textContent=newBal}});
 // 余额自动轮询：每30秒从Gist同步
 setInterval(function(){syncBalanceFromGist(function(ok,newBal){if(ok){var be=document.getElementById('bbal');if(be&&newBal>bal){be.textContent=newBal;be.style.color='#ffd700';setTimeout(function(){be.style.color='#d4b88c'},1500)}}})},30000);
}
...
console.log('[Archive] 充值系统 v5 | 余额',bal,'🧱 | 订单Gist同步 + 余额Gist同步');
})();
